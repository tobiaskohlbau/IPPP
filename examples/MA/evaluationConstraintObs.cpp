...
void runSerial(ModuleConfigurator<6>& sCtr, size_t type) {
    ...
    if (drawImages) {
        ...
        auto workspace2D = cad::create2dspace(sCtr.getEnvironment()->getSpaceBoundary(), 255);
        ...
        drawing::drawSerialRobot2D<6>(start, *serialRobot, image, workspace2D.second, Vector3i(0, 0, 255));
        drawing::drawSerialRobot2D<6>(goal, *serialRobot, image, workspace2D.second, Vector3i(0, 0, 255));
        drawing::imwrite("serial" + std::to_string(type) + ".png", image);
    }
}
    ...
    for (auto& samplingType : samplingTypes) {
        ...
        for (auto& seed : seeds) {
            for (size_t i = 0; i < 3; ++i) {
                ...
                runSerial(sCtr, i);
                ui::save(samplingName + "ObsSerial.json", Stats::serialize(), 4, true);

                // iiwa
                Stats::initializeCollectors();
                auto iCtr = getIiwaCreator(seed, samplingType, i);
                runIiwa(iCtr, i);
                ui::save(samplingName + "ObsIiwa.json", Stats::serialize(), 4, true);
            }
        }
    }
}
