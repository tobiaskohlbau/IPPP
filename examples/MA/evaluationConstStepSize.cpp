...
ModuleConfigurator<6> getSerialCreator(std::string seed, double stepSize, size_t obstacleType) {
    ...
    if (obstacleType == 0) {
        envConfigurator.addObstacle(FLAGS_obstacle2dDir + "100x100.obj", util::Vecd(1100, 800, 0, 0, 0, 0));
        envConfigurator.addObstacle(FLAGS_obstacle2dDir + "100x100.obj", util::Vecd(1100, 1200, 0, 0, 0, 0));
    } else if (obstacleType == 1) {
        envConfigurator.addObstacle(FLAGS_obstacle2dDir + "100x100.obj", util::Vecd(850, 1200, 0, 0, 0, 0));
        envConfigurator.addObstacle(FLAGS_obstacle2dDir + "100x100.obj", util::Vecd(1150, 1200, 0, 0, 0, 0));
    ...
    creator.setEvaluatorProperties(stepSize, 90);
...
void runSerial(ModuleConfigurator<6>& sCtr, size_t type, double stepSize) {
    ...
    auto planner = std::make_shared<RRTStarConnect<6>>(sCtr.getEnvironment(), sCtr.getRRTOptions(stepSize), sCtr.getGraph(),
                                                       sCtr.getGraphB());
    planner->computePath(start, goal, 480, 24);
    planner->getPath();
}

void runIiwa(ModuleConfigurator<7>& iCtr, size_t type, double stepSize) {
    ...
    auto planner = std::make_shared<RRTStarConnect<7>>(iCtr.getEnvironment(), iCtr.getRRTOptions(stepSize), iCtr.getGraph(),
                                                       iCtr.getGraphB());
    planner->computePath(start, goal, 480, 24);
    planner->getPath();
}
    ...
    for (double stepSize = util::toRad(25); stepSize <= util::toRad(150); stepSize += util::toRad(5)) {
        ...
        for (auto& seed : seeds) {
            for (size_t i = 0; i < 3; ++i) {
                ...
                runSerial(sCtr, i, stepSize);
                ui::save("SerialConstStepSize" + stepSizeString + ".json", Stats::serialize(), 4, true);

                // iiwa
                Stats::initializeCollectors();
                auto iCtr = getIiwaCreator(seed, stepSize, i);
                // runIiwa(iCtr, i, stepSize);
                ui::save("IiwaConstStepSize" + stepSizeString + ".json", Stats::serialize(), 4, true);
            }
        }
    }
}
