...
int main(int argc, char** argv) {
    ...
    Vec<float> start(0, 0, 0, 0, 51, 0);
    robot->saveMeshConfig(start);
    Vec<float> goal(150, -60, 90, 0, 82.7, 75.5);
    ...
    if (connected) {
        ...
        std::vector<Vec<float>> pathAngles = planner.getPath(0.3, true);

        std::vector<std::vector<Vec<float>>> vecs;
