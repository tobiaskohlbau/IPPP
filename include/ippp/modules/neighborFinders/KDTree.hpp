...
class KDTree : public NeighborFinder<dim, T> {
  ...
  private:
    ...
    void quickSort(std::vector<T> &A, size_t left, size_t right, int cd);
    size_t partition(std::vector<T> &A, size_t left, size_t right, int cd);
