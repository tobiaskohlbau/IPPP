...
/// Constructor.
        ...
        Frame(const Vec<3,T> &n_, const Vec<3,T> &o_, const Vec<3,T> &a_, const Vec<3,T> &p_){
            ...
            N().X() = n_.X(); N().Y() = n_.Y(); N().Z() = n_.Z(); N().W() = 0;
            O().X() = o_.X(); O().Y() = o_.Y(); O().Z() = o_.Z(); O().W() = 0;
            A().X() = a_.X(); A().Y() = a_.Y(); A().Z() = a_.Z(); A().W() = 0;
            P().X() = p_.X(); P().Y() = p_.Y(); P().Z() = p_.Z(); P().W() = 1;
        }
        ...
        Vec<4,T>& N() {return m_data[0];}
        Vec<4,T>& O() {return m_data[1];}
        Vec<4,T>& A() {return m_data[2];}
        Vec<4,T>& P() {return m_data[3];}
        const Vec<4,T>& N() const {return m_data[0];}
        const Vec<4,T>& O() const {return m_data[1];}
        const Vec<4,T>& A() const {return m_data[2];}
        const Vec<4,T>& P() const {return m_data[3];}
        ...
        /// \return Data pointer.
        T* GetPointer() {return m_data[0].GetPointer();}

        /// Returns a pointer to the data.
        /// Data is stored in the order n,o,a,p.
        /// \return Data pointer.
        const T* GetConstPointer() const {return m_data[0].GetConstPointer();}
        ...
        {
            if (fabs(!N() - 1) > eps) return false;
            if (fabs(!O() - 1) > eps) return false;
            if (fabs(!A() - 1) > eps) return false;
            if (fabs(N() * O()) > eps) return false;
            if (fabs(N() * A()) > eps) return false;
            if (fabs(O() * A()) > eps) return false;
        ...
        {
            if ((P().X() > (f.P().X() + eps)) || (P().X() < (f.P().X() - eps))) return false;
            if ((P().Y() > (f.P().Y() + eps)) || (P().Y() < (f.P().Y() - eps))) return false;
            if ((P().Z() > (f.P().Z() + eps)) || (P().Z() < (f.P().Z() - eps))) return false;
            if ((N().X() > (f.N().X() + eps)) || (N().X() < (f.N().X() - eps))) return false;
            if ((N().Y() > (f.N().Y() + eps)) || (N().Y() < (f.N().Y() - eps))) return false;
            if ((N().Z() > (f.N().Z() + eps)) || (N().Z() < (f.N().Z() - eps))) return false;
            if ((O().X() > (f.O().X() + eps)) || (O().X() < (f.O().X() - eps))) return false;
            if ((O().Y() > (f.O().Y() + eps)) || (O().Y() < (f.O().Y() - eps))) return false;
            if ((O().Z() > (f.O().Z() + eps)) || (O().Z() < (f.O().Z() - eps))) return false;
            if ((A().X() > (f.A().X() + eps)) || (A().X() < (f.A().X() - eps))) return false;
            if ((A().Y() > (f.A().Y() + eps)) || (A().Y() < (f.A().Y() - eps))) return false;
            if ((A().Z() > (f.A().Z() + eps)) || (A().Z() < (f.A().Z() - eps))) return false;
            return true;
        }
        ...
        {
            Frame<T> res = FTrans(pose[0], pose[1], pose[2]) * FRotX(pose[3]) * FRotY(pose[4])
                           * FRotZ(pose[5]);

            N() = res.N(); O() = res.O(); A() = res.A(); P() = res.P();
    ...
    */
        ...
        {
            Frame<T> res = FTrans(pose[0], pose[1], pose[2]) * FRotZ(pose[3]) * FRotY(pose[4])
                           * FRotX(pose[5]);
        ...
        {
            if(fabs(angle) <= EPSILON) return (*this) = Frame();
            Vec<4, T> vn(v);
            vn.W() = 0;
            T vnLength = vn.Length();
            if(vnLength <= EPSILON) return (*this) = Frame();
